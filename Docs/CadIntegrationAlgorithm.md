...
Для стабильной передачи задач из фонового сервиса (Worker) во внешние CAD-приложения используется **асинхронный файловый обмен через «Горячую папку» (File-based Hot Folder Queue) + Idling/Automation события**. 
...
1. Worker контролирует состояние процесса через `Process.WaitForExit(timeout)`.
2. Если процесс завершился по таймауту, Worker убивает его (`Process.Kill()`) и отмечает команду в БД как `Failed` с ошибкой: *"Превышен таймаут ожидания выполнения (зависание процесса)"*.
3. Если процесс упал с системной ошибкой (Exit Code != 0), Worker записывает ошибку: *"Критический сбой процесса CAD. Код выхода: {ExitCode}"*.
