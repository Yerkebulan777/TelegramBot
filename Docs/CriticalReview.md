...
## 5. `Worker:Partitions` — legacy название
...
**Статус:** Legacy совместимость. Переименование сломает существующие `appsettings.json`.
При следующем major-релизе можно переименовать в `Worker:MaxConcurrency`.
