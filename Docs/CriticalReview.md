...
>
> **Проверено:** 2026-07-02 — все 5 пунктов верифицированы против текущего кода. Пункты 1–4
> актуальны (точно описывают текущее состояние), пункт 5 корректно помечен «Исправлено».
> Детали расследования — в комментариях под каждым пунктом.
...
(`TelegramBot.Core/Models/ResultFile.cs:61-62`, атрибут `[XmlElement("outputFiles")]`),
а не массивом — один выходной файл, не список.

**Статус:** Зафиксировано в контракте как **by-design**. Это соответствует канону в
`RevitBIMFusion/Docs/ResultFile.schema.xsd` и явно задокументировано в XML-doc самой модели
(`ResultFile.cs:53-60`). Изменение на массив потребует координации с RevitBIMFusion
(плагин, схемы, парсеры) — не планируется.
