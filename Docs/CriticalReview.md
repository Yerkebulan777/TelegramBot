...
- `NotificationSenderService` читает pending outbox-записи при старте, по wake-up сигналу и периодическим
  polling каждые 30 секунд.
- Отправленная Telegram-сводка помечается `Status='sent'`. При ошибке отправки запись возвращается в
  `pending` с backoff через `NextAttemptAt`.
...
- Если Telegram сообщение успешно отправлено, но запись не успела пометиться `sent` из-за сбоя БД или
  падения процесса, возможен повтор итоговой сводки после retry. Это обычная граница at-least-once
  доставки без внешнего идемпотентного ключа в Telegram API.
