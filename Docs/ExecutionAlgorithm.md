...
`RuntimeSettings.root_path` — единый корень для Server. Любой авторизованный пользователь задаёт его из `/help`, отправляя букву подключённого сетевого диска (`Z:\`), вложенную папку (`Z:\Проекты`) или готовый `\\сервер\шара`. `UncPathResolver` приводит путь к UNC через `WNetGetUniversalName`; сохраняется только UNC, существующий и доступный учётной записи службы Server (проверки: полнота `\\сервер\шара`, `Directory.Exists`, отсутствие reparse-point). Локальные диски не принимаются — резолв для них не даёт UNC. При создании команды это значение записывается в `Commands.RootPath`. Смена глобального значения немедленно влияет на новые выборы файлов, но Worker валидирует queued/processing-команду по её неизменяемому снимку.
