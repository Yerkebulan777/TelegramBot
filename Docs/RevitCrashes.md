...
> ⚠️ **Косвенное опровержение:** `DialogDismisser` ищет окна только у **уже запущенного** процесса
> с известным PID, а краш происходит **до** регистрации процесса в `_activeProcesses`. Однако
> `DialogDismisser` периодически сканирует все активные процессы, и при churn'е процессов теоретически
> может попасть в окно между `Process.Start()` и крашом. Нужен тест на реальных задачах.
