...
ожидания, поэтому несколько `Revit.exe` могут стартовать практически одновременно (в пределах общего
лимита параллельности). Встроенный в Revit Chromium-компонент (CEF) пытается забиндить devtools-порт при
...
- **Цель:** устранить коллизию старта без потери параллелизма выполнения. Команды должны **стартовать по
  очереди**, но **выполняться параллельно** (как и раньше, в рамках общего лимита Worker).
- **Не-цель:** не меняем лимит параллельной работы и не вводим per-процесс
...
- Общий лимит параллельности — без изменений. Gate не уменьшает суммарный параллелизм выполнения,
  только разносит во времени **моменты старта**.
