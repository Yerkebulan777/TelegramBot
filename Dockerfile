...
COPY . .
WORKDIR "/src"
RUN dotnet build "TelegramBot.slnx" -c Release -o /app/build

FROM build AS publish
RUN dotnet publish "TelegramBot.Server/TelegramBot.Server.csproj" -c Release -o /app/publish /p:UseAppHost=false

FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
ENTRYPOINT ["dotnet", "TelegramBot.Server.dll"]
