...
public sealed class WorkerOptions
{
    ...
    /// <summary>
    /// Пауза между запусками внешних процессов в секундах (по умолчанию 30).
    /// Revit держит встроенный Chromium (CEF) для devtools-порта 8088; одновременный
    /// Process.Start() нескольких Revit.exe ведёт к коллизии порта и ACCESS_VIOLATION.
    /// 5с, затем 10с оказались недостаточны на практике — CEF-бинд у предыдущего
    /// Revit иногда происходит позже старта процесса, чем 10с (см. ACCESS_VIOLATION
    /// в логах 2026-07-02 даже при работающем 10с-stagger). Поднято до 30 для максимального
    /// запаса стабильности — ценой более медленного запуска батча параллельных команд.
    /// 0 отключает stagger (старая, нестабильная при параллельном Revit поведение).
    /// </summary>
    public int LaunchStaggerSeconds { get; set; } = 30;
