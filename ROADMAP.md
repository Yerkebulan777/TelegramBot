...
### В планах
- [ ] **Интеграция с CAD-плагинами (Revit/Navisworks) через JSON-обмен** — реализация стабильной передачи заданий по схеме «Один процесс — Одна задача» через файловый JSON-транспорт (`task_{id}.json` / `result_{id}.json`) и перехват событий `Idling` в Revit и `Automation` в Navisworks.
...
|---|-----------|-----------|-----------|----------|
| 1 | **Timing stats в уведомлениях** | ✅ Реализовано | 🟢 Низкая | Уведомление содержит длительность сессии по `MIN(StartedAt)` / `MAX(CompletedAt)` |
| 2 | **Фильтрация в /status** | 🟠 Средний | 🟢 Низкая | При большом количестве сессий список становится нечитаемым. **Решение:** фильтрация по проекту (`ProjectName`) и статусу, пагинация по страницам |
| 3 | **Дневной лимит файлов на пользователя** | ✅ Реализовано | 🟢 Низкая | `RateLimit:MaxFilesPerUserPerDay = 100`; при превышении бот отказывает в создании новой сессии |
| 4 | **Автоочистка старых сессий** | ✅ Реализовано | 🟢 Низкая | Worker мягко удаляет сессии старше `CompletedSessionRetentionDays`, если в них нет `pending`/`processing` |
| 5 | **Умный retry: transient vs permanent** | 🟡 Низкий | 🟡 Средняя | Сейчас retry для всех ошибок одинаков. **Решение:** классифицировать по exit code / error message: `InvalidFileError` → сразу Failed (retry бесполезен), `ProcessCrashError` → retry (возможно временный сбой) |
| 6 | **Confirmation dialogs для удаления** | ✅ Реализовано | 🟢 Низкая | `DELETESESSION:` / `DELETECOMMAND:` сначала показывают подтверждение, затем soft-delete |
| 7 | **Асинхронное ожидание процессов** | ✅ Реализовано | 🟢 Низкая | В .NET 10 используется асинхронное ожидание `await process.WaitForExitAsync(ct)` вместо блокирующего `WaitForExit()` внутри `Task.Run` для разгрузки ThreadPool |
| 8 | **Асинхронный сбор файлов (RVT)** | 🟠 Средний | 🟢 Низкая | В `SlashCommandService.CollectRvtFiles` заменить синхронный обход ФС на асинхронный, чтобы не блокировать цикл обработки Telegram-сообщений |
| 9 | **Persistent RateLimiter** | 🟡 Низкий | 🟡 Средняя | Перенести хранение окон запросов из `ConcurrentDictionary` в PostgreSQL/Redis для сохранения лимитов после перезагрузки сервера |
