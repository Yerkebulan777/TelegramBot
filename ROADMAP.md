...
- [x] **Оптимизация: in-memory счётчик сессий** — удалён per-command `GetSessionProgressAsync`,
  заменён на `ConcurrentDictionary.AddOrUpdate`.
...
| 2 | **Timing stats в уведомлениях** | 🟠 Средний | 🟢 Низкая | Добавить в уведомление общее время сессии: `✅ ProjectA — 3 мин 12 с — все 5 файлов обработано`. Вычисляется через `MIN(StartedAt)` / `MAX(CompletedAt)` из таблицы Commands по SessionId — не путать с `GetSessionsStatusAsync`, который не содержит временных меток |
...
| 4 | **Дневной лимит файлов на пользователя** | 🟠 Средний | 🟢 Низкая | Защита от случайного создания слишком большой очереди. **Решение:** `MaxFilesPerUserPerDay = 100` — при превышении дневного лимита бот отказывает в создании новой сессии с понятным сообщением |
| 5 | **Автоочистка старых сессий** | 🟡 Низкий | 🟢 Низкая | Сессии `Done`/`Failed` старше N дней автоматически помечать `Deleted`. **Решение:** фоновая задача с настраиваемым порогом в `appsettings.json` |
...
| 7 | **Confirmation dialogs для удаления** | 🟡 Низкий | 🟢 Низкая | Сейчас удаление сессии/команды — без подтверждения. **Решение:** добавить диалог (по аналогии с отменой: `DELETESESSION:{id}` → `CONFIRM_DELETE:{id}`) |
