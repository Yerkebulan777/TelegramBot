...
Предупреждения о превышении лимита запросов, анонимном профиле и отклонённые входящие сообщения регистрируются для очистки. Предупреждение о лимите отправляется пользователю не чаще одного раза за `RateLimit:WindowSeconds`, включая нажатия кнопок. Устаревший интерфейс удаляется при следующем взаимодействии, а неудачное удаление сохраняется для повтора. Итоговые уведомления защищены от `/status`, `/export` и другой интерактивной очистки.

`MessageCleanup`: `IntervalMinutes=1` — проверка удаления; `RetentionHours=24` — хранение результатов и неочищенного интерфейса; `TemporaryRetentionMinutes=5` — временные предупреждения и сообщение старта; `BatchSize=500` — записей за цикл; `MaximumDeletionAgeHours=47` — предельный возраст попытки; `Enabled=true`. Явные значения в локальной конфигурации имеют приоритет над этими defaults. Удаления выполняются пакетами до 100; сбой откладывает повтор минимум на минуту с учётом Telegram `retry_after`. После предельного возраста удаляется только запись учёта с Warning: старое сообщение в Telegram остаётся. При обновлении старые записи без типа консервативно защищаются до обычного срока хранения, поскольку отличить старое меню от результата по прежней схеме нельзя.

После доставки результата сообщения этого задания «поставлено в очередь» и «запущено» автоматически становятся кандидатами на удаление, включая поздно сохранённый ID сообщения очереди. Активное меню другого задания это не затрагивает. При обновлении установите совместно новые Server и Worker: сначала остановите службы, затем запустите обновлённый Server для автоматической миграции схемы, после её завершения — Worker.

Уведомления о старте и результате сохраняются в PostgreSQL. Server читает очередь каждые 3 секунды; временная ошибка не останавливает цикл. После аварийного истечения lease итоговое событие создаётся вместе с `Failed`. Раз в минуту Server восстанавливает пропущенные события завершения. Устаревшее сообщение старта не отправляется после завершения задания. Временные ошибки повторяются с задержкой до 5 минут (для 429 — по `retry_after`), постоянные Telegram 400/403 отмечаются в outbox как `failed`. Результат доставки и ID сообщения сохраняются одной транзакцией; при временном сбое этой записи повторяется запись, а не отправка. Авария между ответом Telegram и сохранением в БД всё ещё может привести к дублю. Логи `Notification cycle failed`, `Notification delivery failed`, `Notification acknowledgement deferred` и `Recovered missing completion notifications` различают эти этапы.
...
Worker проходит не менее 15 секунд. Ожидание очереди запуска не ограничено стандартным
30-секундным таймаутом Npgsql, но отменяется общим лимитом выполнения команды; сбой gate до
`Process.Start()` проходит обычный retry. Другие исполняемые файлы этой паузы не ждут.
...
запуска Revit. Запись terminal-статуса и создание итогового outbox-уведомления выполняются в одной
транзакции. При исчерпании попыток ошибка фиксируется отдельно; ResultFile сохраняется.
